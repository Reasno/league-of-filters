...
exports.twoWayFilterAsyncTimeout = (predicate, errFormat, timeout, fallbackAs) => (source) => {
    let count = 0;
    ...
    const tested$ = source.pipe(operators_1.mergeMap((data) => {
        return rxjs_1.from(predicate(data, count++))
            .pipe(operators_1.timeoutWith(timeout, rxjs_1.of({ filterResult: fallbackAs, entry: data, timeout: true })), operators_1.map(isValid => ({ filterResult: isValid, entry: data, timeout: false })));
    }));
    const [noTimeout$, timeout$] = rxjs_1.partition(tested$, (data) => data.timeout === false);
    ...
    sr.timeout = rxjs_1.merge(sr.timeout, timeout$);
